...
- **Creation Hub**: AI-powered content generation platform at `/creation-hub` with brand profile management (colors, style keywords, aspect ratio preferences), custom prompt templates, and generation job tracking. Supports image and video generation with provider selection (OpenAI, Stable Diffusion). Features tabbed interface: Create (generate content), Templates (manage prompt templates), Brand (visual identity settings), and History (generation jobs). Database tables: `user_brand_profiles`, `prompt_templates`, `generation_jobs`.
